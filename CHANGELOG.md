...
### Changed
- Run the CI test job on Node.js 20.x, 22.x and 24.x instead of 20.x only. Which modules are built in is now the runtime's answer rather than a table inside use-m, so the runtime version is part of the contract and every supported Node.js line has to run the suite. Bun and Deno execute the tests with their own runtime and stay pinned to a single Node.js for `npm ci`, so the matrix grows by four jobs instead of twelve; `tests/release-workflow-policy.test.mjs` asserts that shape.
