...
describe('[MJS Runtime] resolvers tests', () => {
  test('[MJS Runtime] npm resolver resolves package path', async () => {
    const { npm } = resolvers;
  ...
  test('[MJS Runtime] npm resolver resolves scoped package path for @octokit/core@6.1.5', async () => {
    const { npm } = resolvers;
    const packagePath = await npm('@octokit/core@6.1.5', resolve);
    expect(packagePath).toMatch(/node_modules\/octokit-core-v-6\.1\.5/);
  });

  test('[MJS Runtime] npm resolver resolves package path with version', async () => {
    const { npm } = resolvers;
  ...
  test('[MJS Runtime] npm resolver resolves yargs/helpers', async () => {
    const { npm } = resolvers;
  ...
  test('[MJS Runtime] jsdelivr resolver resolves subpath', async () => {
    const { jsdelivr } = resolvers;
  ...
  test('[MJS Runtime] unpkg resolver resolves URL', async () => {
